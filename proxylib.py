...
class UDPHandler(BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        logging.info("Registrovany pouzivatel: %s - Adresa: %s" % (fromm, contact))
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar:
                ...
                showtime()
                if data[0][:3] == "INV":
                    data[0] = "Ucastnik " + origin + " zvoni " + destination
    ...
    def processAck(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar:
                ...
                showtime()
                if data[0][:3] == "ACK":
                    data[0] = "Hovor prijaty medzi " + origin + " a " + destination
                logging.info("<<< %s" % data[0])
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text), text))
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar:
                ...
                showtime()
                if data[0][:3] == "BYE":
                    data[0] = "Hovor ukonceny "
                    final = "medzi " + origin + " a " + adresat
                logging.info("<<< %s" % data[0] + final)
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text), text))
            else:
